...
def etl_geovane_base_original():
    ...
    WHERE WorkOrder__r.CreatedDate >= 2025-10-01T00:00:00.000-03:00
    """
    ...
    # Remove colunas attributes do Salesforce
    all_results = all_results.drop(
        columns=[c for c in all_results.columns if 'attributes_' in c],
        errors='ignore'
    )

    # Evita FutureWarning do pandas
    all_results = all_results.astype(object).fillna("")
    ...
    # 🔥 CORREÇÃO DEFINITIVA – REMOVE COLUNAS RELACIONAIS DO SALESFORCE
    df_renomeado = df_renomeado.drop(
        columns=[c for c in df_renomeado.columns if c.startswith('WorkOrder__r_')],
        errors='ignore'
    )

    # Evita FutureWarning
    df_renomeado = df_renomeado.astype(object).fillna("")
