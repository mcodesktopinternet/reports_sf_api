...
logger = logging.getLogger("etl_richard")
...
WHERE WorkOrder__r.CreatedDate >= {CREATED_FROM}
  AND WorkOrder__r.Priority = 'Critical'
...
def etl_richard_critical():
    ...
    created_from = os.getenv("SF_CRITICAL_CREATED_FROM", "2025-08-11T00:00:00.000-03:00")
    ...
    logger.info("Consultando Salesforce... (CreatedDate >= %s | Priority=Critical)", created_from)
    ...
    try:
        ...
        logger.info("Limpando tabela servicos_tecnicos...")
        cursor.execute("TRUNCATE TABLE servicos_tecnicos;")
        conn.commit()

        logger.info("Inserindo dataframe no MySQL... (%s linhas)", len(df))
        ok = insert_dataframe_mysql_direct(df, "servicos_tecnicos", conn)
